...
#warning "Change to SDK include"
// XXX: application file location
#include <freertos/nv/nvintf.h>
#include <freertos/nv/nvocmp.h>
...
const CC13X2_26X2Config::Key CC13X2_26X2Config::kConfigKey_OperationalDeviceICACerts   = { { kCC13X2_26X2ChipFactory_Sysid, 0x001c } };
const CC13X2_26X2Config::Key CC13X2_26X2Config::kConfigKey_OperationalDevicePrivateKey = { { kCC13X2_26X2ChipFactory_Sysid, 0x001d } };
...
CHIP_ERROR CC13X2_26X2Config::ReadConfigValueBin(Key key, uint8_t * buf, size_t bufSize, size_t & outLen)
{
    ...
    VerifyOrExit(sNvoctpFps.readItem(key.nvID, 0, len, buf) == NVINTF_SUCCESS, err = CHIP_ERROR_PERSISTED_STORAGE_FAILED);
...
CHIP_ERROR CC13X2_26X2Config::WriteConfigValueBin(Key key, const uint8_t * data, size_t dataLen)
{
    ...
    VerifyOrExit(sNvoctpFps.writeItem(key.nvID, dataLen, data) == NVINTF_SUCCESS, err = CHIP_ERROR_PERSISTED_STORAGE_FAILED);
exit:
    return err;
}
